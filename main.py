...
def invalidSession():
  resp = make_response(
    render_template('redirect.html', redirect_location="/login"))
...
def login():
  ...
  if isSession(token) and request.method == "GET" and token != None:
    ...
    resp = make_response(
      render_template('redirect.html', redirect_location="/challenge"))
...
def registerForm():
  ...
  try:
    ...
    username = str(
      personalFunctions.base64decode(
        request.args.get('uname')).decode("utf-8"))
    email = str(
      personalFunctions.base64decode(
        request.args.get('email')).decode("utf-8"))
    password = str(
      personalFunctions.base64decode(request.args.get('psw')).decode("utf-8"))
    ...
    if "@" not in email or "." not in email:
      return personalFunctions.base64encode(
        personalFunctions.replaceNewlines(
          "Please enter a valid email").encode())

    # check if there is a username conflict
    if accountManager.getAccountByUsername(username) != None:
      return personalFunctions.base64encode(
        personalFunctions.replaceNewlines(
          "Sorry, that username exists!").encode())

    # check if there is an email conflict
    if accountManager.getAccountByEmail(email) != None:
      return personalFunctions.base64encode(
        personalFunctions.replaceNewlines(
          "Sorry, that email is in use by another account!").encode())
    ...
    if verification != None:
      return personalFunctions.base64encode(
        personalFunctions.replaceNewlines(
          "You have already been sent an email").encode())
    ...
    return personalFunctions.base64encode(
      personalFunctions.replaceNewlines(
        "Please check your email for a code to input. It may be in the spam folder."
      ).encode())
  except:
    # this is returned if there was an error in anything above
    return personalFunctions.base64encode(
      personalFunctions.replaceNewlines(
        "There has been an error processing your information. Please try again now or later."
      ).encode())
...
def admin():
  ...
  if request.method == "POST":
    if (request.form['button'] == 'saveAccounts'):
      ...
      yml.reloadChallenges()
  resp = make_response(
    render_template('admin.html', challenges=yml.rawChallengesData))
  resp = setHeaders(resp, currentSession.token)
  return resp
...
def challenge():
  ...
  if account.preSurvey == accountManager.surveyDict:
    resp = make_response(
      render_template('redirect.html', redirect_location='/presurvey'))
    resp = setHeaders(resp, currentSession.token)
    return resp
  ...
  resp = make_response(
    render_template('challenge.html',
                    username=currentSession.username,
                    data=yml.data,
                    courseCompletion=courseCompletions[account.uid],
                    free=account.free))
  resp = setHeaders(resp, currentSession.token)
  return resp
...
def submitCompletion():
  ...
  try:
    ...
    page = personalFunctions.base64decode(
      request.args.get('page_id')).decode('utf-8')
    status = personalFunctions.base64decode(
      request.args.get('status')).decode('utf-8')
...
def recieve_code():
  ...
  try:
    ...
    if currentSession == None:
      return personalFunctions.base64encode(
        "<p><a href=\"login\">Please log in</a></p>".encode())

    # decode the code argument from base64
    program = personalFunctions.base64decode(
      request.args.get('code')).decode('utf-8')

    # get the user's code from the request
    output = personalFunctions.base64decode(
      request.args.get('output')).decode('utf-8').replace("<br>", "\n")
    #print(output)
    ...
    pageName, question, chal_id = personalFunctions.base64decode(
      request.args.get('chal_id')).decode('utf-8').split(" ")
    ...
    if yml.data[pageName]['page'][question]["chal_id"] == chal_id:
      ...
      if yml.data[pageName]['page'][question]["correct"] + "\n" == output:
        ...
        return personalFunctions.base64encode(
          personalFunctions.replaceNewlines(
            "<p class=\"correct\">Correct!</p>").encode())
      # if the user is expeced to change the code, then compare thier code to the originial skeleton
        ...
        if yml.data[pageName]['page'][question]["skeleton"] != program:
          ...
          return personalFunctions.base64encode(
            personalFunctions.replaceNewlines(
              "<p class=\"incorrect\">Incorrect! Try again.</p>").encode())
  ...
  # if there was an erorr anywhere, when display this error
    ...
    return personalFunctions.base64encode(
      personalFunctions.replaceNewlines(
        "<p class=\"incorrect\">There has been an erorr in saving your code. Please try again now or later.</p>"
      ).encode())
...
def get_chall(id):
  ...
  if account == None:
    resp = make_response(
      render_template('redirect.html', redirect_location='/logout'))
    resp = setHeaders(resp, currentSession.token)
    return resp
  # try to load the page for the user. If there is an erorr, that means the user has not completed the presurvey
  try:
    return render_template("challengeTemplate.html",
                           data=yml.data,
                           page=id,
                           completion=completions[account.uid],
                           courseCompletion=courseCompletions[account.uid])
  except:
    resp = make_response(
      render_template('redirect.html', redirect_location='/presurvey'))
...
def verify(id):
  ...
  if verify == None:
    resp = make_response(
      render_template('redirect.html', redirect_location='/register'))
    return resp
...
def preSurvey():
  ...
  if account.preSurvey != accountManager.surveyDict:
    resp = make_response(
      render_template('redirect.html', redirect_location="/challenges"))
...
def postSurvey():
  ...
  if request.method == "POST":
    ...
    resp = make_response(
      render_template('redirect.html', redirect_location="/results"))
    resp.set_cookie('token', currentSession.token)
    return resp
...
def results():
  ...
  resp = make_response(
    render_template('results.html',
                    preSurvey=account.preSurvey,
                    postSurvey=account.postSurvey,
                    preFeeling=survey.preFeeling,
                    prePursue=survey.prePursue,
                    postFeeling=survey.postFeeling,
                    postPursue=survey.postPursue))
  resp = setHeaders(resp, currentSession.token)
  return resp
...
def header():
  ...
  if isSession(token) and request.method == "GET" and token != None:
    ...
    resp = make_response(
      render_template('header.html', login=True, admin=account.admin))
    resp = setHeaders(resp, currentSession.token)
    return resp
